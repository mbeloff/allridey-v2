<template>
  <div class="rounded bg-opacity-90 w-full p-1 flex flex-col gap-5">
    ...
    <!-- // ? LEFT SIDE -->
      <div class="w-full md:w-2/6 text-left text-sm shadow-xl">
        ...
        <div v-if="totals.all.length > 0" class="bg-blue-900 text-white px-2 py-4 rounded-b">
          ...
          <div class="text-right italic text-xs" v-if="totals.tax[0].total">
            <span v-for="tax in totals.tax">(includes GST of: {{' ' + currencysymbol + totals.tax[0].total}})</span>
          </div>
        </div>
      </div>
    ...
    <make-booking @create-booking="setRef" @modeChange="changeMode" :key="count" :optionalfees="optionalfees" :searchParams="searchParams" :calcTotals="calcTotals" :step3="step3"></make-booking>
  </div>
</template>
