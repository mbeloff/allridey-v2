<template>
  <div class="p-10 text-center">
    <div class="w-full h-full grid place-items-center">
      ...
      <p>Allridey offers great value car and van rentals with {{ locationcount }} locations around Australia. We've got the best prices - get an instant quote and book online today.</p>
    </div>
  </div>
  <div class="grid grid-cols-1 md:grid-cols-3 gap-x-0 gap-y-10 md:gap-10">    
  <div v-for="(article, index) in articles" :key="index" class="w-full text-left rounded" :class="article.cols">
    <div class="bg-white rounded shadow-lg group">
       <div class="h-48 rounded-t bg-cover bg-center relative" :style="{'background-image' : 'url(' + article.img + ')'}">
